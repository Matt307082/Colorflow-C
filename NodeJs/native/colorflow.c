...
pixel* createPixel(unsigned char R, unsigned char G, unsigned char B, unsigned char A){
  ...
  if (!p) {
        strLastErrorMessage = (char*)"Error while allocating memory for pixel.";
        return NULL;
  }
  p->red = R;
  p->green = G;
...
char* pixelToHex(pixel* myPixel){
  ...
  int sizeCreatedStr = sprintf(average_HEX, "%02X%02X%02X-%02X", myPixel->red, myPixel->green, myPixel->blue, myPixel->alpha);
  if(sizeCreatedStr != 9){
